...
public class JdbcPostDao implements PostDao{
	...
	public List<Post> getPostsByUserId(int id) {
		List<Post> thePosts = new ArrayList<Post>();
		String sqlSelectAllPosts = "SELECT * FROM post, post_user_reference WHERE post.post_id = post_user_reference.post_id AND post_user_reference.id = ?";
		SqlRowSet results = jdbcTemplate.queryForRowSet(sqlSelectAllPosts, id);
	...
	public boolean savePost(Post newPost, int user_id) {
		...
		try{
			int id = getNextId();
			String sqlInsertPost = "INSERT INTO post(id, title, img_url, thumbnail, tags) VALUES (?,?,?,?,?)";
			String sqlInsertUserPostRef = "INSERT INTO post_user_reference(post_id, id) VALUES (?,?)";
			jdbcTemplate.update(sqlInsertPost, id, newPost.getTitle(), newPost.getImg_url(), newPost.getThumbnail(), tagStr);
			jdbcTemplate.update(sqlInsertUserPostRef, id, user_id);
		} catch (Exception e){
			return false;
		}
		return true;
	...
	private Post mapRowToPost(SqlRowSet row) {
		...
		thePost.setThumbnail(row.getString("thumbnail"));
